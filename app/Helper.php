...
function inititate_mobile_transaction($msisdn,$amount,$request){
  $uri = 'https://vendors.pay-leo.com/api/make_transaction.php';
  $merchantCode = '78554';
  $consumerKey	= 'I2zqctRHBi8875udvnwTD48kv51486987332';
  $consumerSecret = '5ckweSlR3DiBkGQg2bhHoIhsiJ1486987332';
  $transactionId = $request;
  $narration	=	'Payment for ride '.$request;
  $data = $uri.'&'.$msisdn.'&'.$amount.'&'.$merchantCode.'&'.$transactionId.'&'.$narration;
...
function process_mobile_money_transaction($msisdn,$amount){
  $url = \Config::get('yo.url');
  'https://paymentsapi2.yo.co.ug/ybs/task.php';
  $xml = '<?xml version="1.0" encoding="UTF-8"?>
          <AutoCreate>
              <Request>
                  <APIUsername>'.\Config::get('yo.username').'</APIUsername>
                  <APIPassword>'.\Config::get('yo.password').'</APIPassword>
                  <Method>acdepositfunds</Method>
                  <NonBlocking></NonBlocking>
                  <Amount>'.$amount.'</Amount>
                  <Account>'.$msisdn.'</Account>
                  <Narrative>Test Withdrawal</Narrative>
              </Request>
          </AutoCreate>';
  $ch = curl_init();
  ...
  $status = $array['Response']['StatusCode'];
  return $status;
}

function broadcast_fcm_notification($topic, $message){
  $fields = array(
    'to' => '/topics/'.$topic,
    'notification' => array('title' => 'QuickTaxi', 'body' => $message),
    'data' => array('message' => $message,'team' => null)
  );
  $headers = array(
    'Authorization:key='.Config::get('fcm.key'),
    'Content-Type:application/json'
...
function send_fcm_notification($token, $data){
  $fields = array(
    'to' => $token,
    //'notification' => array('title' => 'QuickTaxi', 'body' => $message),
    'data' => $data
...
function send_notifications($id, $type, $action, $message, $is_imp = NULL) {
...
function send_sms_message($phone, $message){
  ...
  $username = "dev@quicktaxi.ug";
  $password = "quick1234";
  $url = "https://api.dmarkmobile.com/v1/blasta.php?spname=".$username."&sppass=".$password."&from=8008&numbers=".$phone."&msg=".$message;
  //file_get_contents($url);
}

function calculate_promo_debt($distance, $time, $type){
  ...
  $providertype = ProviderType::where('id', $type)->first();
  $total = 0;
  if($distance <= 0.5 || $time <= 2){
    $total = 0;
  }elseif($distance > $free_distance){
    $total = $free_amount;
  }else{
    $price_for_distance = 0;
    if($distance > $providertype->base_distance){
      $price_for_distance = $providertype->price_per_unit_distance * ($distance - $providertype->base_distance);
    }
    $price_for_time = $providertype->price_per_unit_time * $time;
    $total = $price_for_distance + $price_for_time + $providertype->base_price;
...
function calculate_client_debt($distance, $time, $type){
  $free_promo = Settings::where('key', 'free_trip_promo')->first()->value;
  $free_amount = Settings::where('key', 'free_promo_amount')->first()->value;
  $free_distance = Settings::where('key', 'free_promo_distance')->first()->value;
  $providertype = ProviderType::where('id', $type)->first();
  ...
  if($distance > $free_distance){
    $price_for_distance = $providertype->price_per_unit_distance * ($distance - $free_distance);
    $price_for_time = $providertype->price_per_unit_time * (($time/$distance) * ($distance - $free_distance));
    $total = $price_for_distance + $price_for_time;
  }
  return $total;
}
...
function transl($key, $id, $user_type) {

    if ($user_type == 0)
        $language = Owner::where('id', $id)->sum('language');
    if ($user_type == 1)
        $language = Walker::where('id', $id)->sum('language');
    if ($language == 0)
        $nm = "en";
    if ($language == 1)
        $nm = "es";
    return Lang::choice('messages.' . $key, $language, Array(), $nm);
}
...
function tran($key) {
  ...
  if(Session::has('locale')){
    ...
    if (Session::get('locale') == "en") {
        ...
        return Lang::choice('messages.' . $key, 1, Array(), Session::get('locale'));
    }
  }
  else{
        Session::put('locale',"en");
...
function currency_converted($total) {
    ...
    if ($currency_sel != 'UGX') {
        ...
        if (!$check) {
            $url = "http://currency-api.appspot.com/api/UGX/" . $currency_sel . ".json?key=65d69f1a909b37e41272574dcd20c30fb2fbb06e";
...
function generate_token() {
    return $this->clean(Hash::make(rand() . time() . rand()));
}
...
function email_notification($id, $type, $message_body, $subject, $trip = null, $is_imp = null, $file_path = null) {
    ...
    if ($email_notification == 1 || $is_imp == "imp") {
        ...
        if ($language != 0) {
            if ($trip == 'invoice') {
                ...
                try {
                    if ($file_path == null) {
                        Mail::send('emails.email_user_invite_event', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                            $message->to($email)->subject($subject);
                        });
                    } else {
                        Mail::send('emails.email_user_invite_event_1', array('mail_body' => 'Invitation Code in attached file.'), function ($message) use ($email, $subject, $file_path) {
                            $message->to($email)->subject($subject)->attach($file_path);
                        });
                    }
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
        ...
        if ($language == 1) {
            ...
            if ($trip == 'invoice') {
                try {
                    Mail::send('emails_es.invoice', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.approve_walker_mail', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.decline_walker_mail', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.reset_password', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.email_new_request', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.email_request_unanswered', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.email_owner_new_register', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.email_walker_new_register', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
                ...
                try {
                    Mail::send('emails_es.email_owner_request_accept_by_driver', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                        $message->to($email)->subject($subject);
                    });
                } catch (Services_Twilio_RestException $e) {
                    //Log::error($e->getMessage());
...
/*function send_notifications($id, $type, $title, $message, $is_imp = NULL) {
    ...
    if ($push_notification == 1 || $is_imp == "imp") {
        if ($user->device_type == 'ios') {
            WARNING:- you can't pass devicetoken as string in GCM or IOS pushyou have to pass array of devicetoken even thow it's only one device's token.
