...
use Illuminate\Support\Facades\Theme;
...
class AdminController extends Controller {
    ...
    public function add_admin_do() {
        ...
        if ($validator->fails()) {
            ...
            $admin->save();
            return Redirect::to("/admins?success=1");
        }
    }
    ...
    public function delete_document_type() {
        ...
        return Redirect::to("/document-types");
    }
    ...
    public function update_document_type() {
        ...
        return Redirect::to("/document-type/edit/$document_type->id?success=1");
    }
    ...
    public function update_info_page() {
        ...
        return Redirect::to("/information/edit/$information->id?success=1");
    }
        ...
        if (!Admin::count()) {
            ...
            if (Auth::attempt(array('username' => $username, 'password' => $password))) {
                if (Session::has('pre_admin_login_url')) {
                    ...
                    //return Redirect::to('/report')->with('notify', 'installation Notification');
                    //return Redirect::to('/report');
                    //return "louis";
    ...
    public function approve_walker() {
        ...
        $user_type = 1;
        $title = transl('you_approved',$id,$user_type);
        ...
        $pattern = array('walker_name' => $walker->first_name . " " . $walker->last_name, 'contact_us_email' => $admin_email);
    ...
    public function decline_walker() {
        ...
        $user_type = 1;
        $title = transl('you_decline',$id,$user_type);
        $message = $response_array;
        //send_notifications($id, "walker", $title, $message, "imp");
        /* SMS */
    ...
    public function walker_history() {
        $walker_id = Request::segment(3);
        $walks = DB::table('request')
                ->where('request.confirmed_walker', $walker_id)
                ->where('request.is_completed', 1)
                ->leftJoin('walker', 'request.confirmed_walker', '=', 'walker.id')
                ->leftJoin('owner', 'request.owner_id', '=', 'owner.id')
                ->select('owner.first_name as owner_first_name', 'owner.last_name as owner_last_name', 'walker.first_name as walker_first_name', 'walker.last_name as walker_last_name', 'walker.phone as walker_phone', 'owner.id as owner_id', 'walker.id as walker_id', 'request.id as id', 'request.created_at as date', 'request.is_started', 'request.is_walker_arrived', 'request.is_completed', 'request.is_paid', 'request.is_walker_started', 'request.confirmed_walker', 'request.status', 'request.time', 'request.distance', 'request.total', 'request.is_cancelled', 'request.payment_mode')
		...
		public function walker_time_logs() {
			$walker_id = Request::segment(3);
			$walker = Walker::find($walker_id);
			$timelogs = DB::table('online_time')->where('driver', $walker_id)->orderBy('online_time.id', 'desc')->paginate(10);
			$title = ucwords(trans('customize.Provider') . ' History' . " : " . $walker->first_name . " " . $walker->last_name);
			$setting = Settings::where('key', 'transfer')->first();
			foreach($timelogs as $key => $timelog){
				if(($timelog->online_time/60) > 5){
					$timelog->amount = 5000;
				}else{
					$timelog->amount = 0;
				}
			}
			return View::make('walker_time_logs')->with('title', $title)->with('page', 'walkers')
				->with('setting', $setting)->with('walker', $walker)->with('logs', $timelogs);
    }
    ...
    public function update_owner() {
        ...
        $owner->save();
        return Redirect::to("/user/edit/$owner->id?success=1");
    }
    ...
    public function owner_history() {
        ...
        $walks = DB::table('request')
                ...
                ->orderBy('request.created_at')
                ->paginate(10);
        $title = ucwords(trans('customize.User') . " History"); /* 'Trip History' */
        foreach ($walks as $walk) {
    ...
    public function save_changed_walker() {
        ...
        return Redirect::to('/walk/change_walker/' . $walk_id);
    }
    ...
    public function pay_walker() {
        ...
        return Redirect::to('/walk/map/' . $walk_id);
    }
    ...
    public function save_keywords() {
        ...
        return Redirect::to('/edit_keywords?success=1');
    }
    ...
    public function save_developer_details() {
        ...
        return Redirect::to('/settings/installation?success=1');
    }
    ...
    public function save_keywords_UI() {
        ...
        $appfile = fopen(app_path() . "/lang/" . Config::get('app.locale') . "/customize.php", "w") or die("Unable to open file!");
    ...
    public function save_settings() {
        ...
        return Redirect::to('/settings?success=1');
    }
...
//Installation Settings
    ...
    public function finish_install() {
        ...
        return Redirect::to('/settings?success=1')->with('install', $install);
    }
...
";
                ...
                return Redirect::to('/settings/installation?success=3')
                                ->with('install', $install);
            }
        }
...
";
                ...
                return Redirect::to('/settings/installation?success=4')
                                ->with('install', $install);
            }
        }
        ...
        if ($is_certy_change > 0) {
            return Redirect::to('/settings/installation?success=1')->with('install', $install);
        } else {
            return Redirect::to('/settings/installation?success=5')
                            ->with('install', $install);
        }
    }
    ...
    public function sortpv() {
        ...
        if ($type == 'provid') {
            ...
            $providers = DB::table('walker')
                    ...
                    /* ->where('walker.is_deleted', 0) */
                    ->where('walker.deleted_at', '=', NULL)
                    ->orderBy('walker.online_time', $valu)
                    ->paginate(10);
    ...
    public function payment_details() {
		...
		public function get_notifications() {
			$notifications = DB::table('notifications')
				...
				->paginate(15);
			return View::make('push_notifications')
				->with('title', 'Push Notifications')
				->with('notifications', $notifications)
				->with('page', 'push_notifications');
		}
	
		public function send_notifications() {
			$topic = Input::get('topic');
      $message = Input::get('message');
			broadcast_fcm_notification($topic, $message);
			DB::table('notifications')->insert(['topic' => $topic, 'message' => $message]);
